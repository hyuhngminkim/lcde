...
class Estimator {
  ...
  void train() {
    ...
    std::vector<LCD> lcds(num_leaf);
    std::vector<std::vector<mpf>> data_leaf(num_leaf);

    auto max = data_.front();
    auto min = data_.back();
    mpf slope = 1. / (max - min);
    mpf intercept = -slope * min;

    slope *= num_leaf - 1;
    intercept *= num_leaf - 1;

    for (const auto &d : data_) {
      int rank = static_cast<int>(slope * d + intercept);
      rank = std::max(0, std::min(num_leaf - 1, rank));
      data_leaf[rank].push_back(d);
    }

    for (const auto &v : data_leaf) {
